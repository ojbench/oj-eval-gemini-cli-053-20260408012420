...
public:
  ...
  */
  bool Check(const std::string &str) const {
    ...
    current_states = nfa.GetEpsilonClosure(current_states);
    for (char c : str) {
  ...
  RegexChecker(const std::string &regex) {
    ...
    for (const std::string &alt : alternatives) {
      ...
      for (size_t i = 1; i < items.size(); ++i) {
        alt_nfa = Concatenate(alt_nfa, items[i]);
      }
      nfa_alts.push_back(alt_nfa);
    }

    if (!nfa_alts.empty()) {
      nfa = nfa_alts[0];
      for (size_t i = 1; i < nfa_alts.size(); ++i) {
        nfa = Union(nfa, nfa_alts[i]);
      }
    }
  }
};
